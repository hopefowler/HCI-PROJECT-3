...
class OxyCSBot(ChatBot):
    ...
    def get_reasoning(self, reasoning):
        ...
        responses = {
            ...
            'bad_response': 'Sorry your roommate sucks. :( What are you looking for from me?',
            'incompatible': 'How can I best help you with this?',
            'no': 'Seems like you got it all figured out, stop wasting my time haha',
            'yes': 'Got it. What are you looking for from me?',
        ...
        if 'space_imbalance' in tags:
            for fridge_issue in self.FRIDGE_ISSUE:
                if fridge_issue in tags:
                    ...
                    return self.go_to_state('fridge_issue')
        elif 'smelly' in tags:
            for fridge_issue in self.FRIDGE_ISSUE:
                if fridge_issue in tags:
                    self.fridge_issue = fridge_issue
    ...
    def on_enter_fridge_issue(self):
        ...
        response = self.get_fridge_issue(self.fridge_issue) + ' Have you had a conversation about this with your roommate and were they receptive?'
        return response
        ...
        if 'no' in tags:
            for conversation_response in self.CONVERSATION_RESPONSE:
                if conversation_response in tags:
                    ...
                    return self.go_to_state('conversation_response')
    ...
    def on_enter_conversation_response(self):
        response = [self.get_conversation_response(self.conversation_response)]

        return response

    def respond_from_conversation_response(self, message, tags):
        self.reasoning = None
        if 'RA_help' in tags:
            for reasoning in self.REASONING:
                if reasoning in tags:
                    self.reasoning = reasoning
                    return self.go_to_state('reasoning')
        elif 'scared' in tags:
            for reasoning in self.REASONING:
                if reasoning in tags:
                    self.reasoning = reasoning
        ...
        if 'rant' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    ...
                    return self.finish('looking_for_rant')
        elif 'advice' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    self.looking_for = looking_for
                    return self.finish('looking_for_advice')
        elif 'solution' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    self.looking_for = looking_for
                    return self.finish('looking_for_solution')
        elif 'room_change' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    self.looking_for = looking_for
                    return self.finish('looking_for_room_change')
        elif 'roommate_issue' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    self.looking_for = looking_for
                    return self.finish('looking_for_roommate_issue')
        elif 'personal_issue' in tags:
            for looking_for in self.LOOKING_FOR:
                if looking_for in tags:
                    self.looking_for = looking_for
                    return self.finish('looking_for_personal_issue')
        else:
            return self.finish('confused')
    ...
    def finish_looking_for_rant(self):
        return 'I am here to listen! Tell me what is on your mind!'

    def finish_looking_for_advice(self):
        return 'Here are some tips on how to have healthy conversations with roommates! ' + 'I recommend the ' \
               'LARA method: L is for LISTENING (make sure to ACTIVELY listen which means ' + 'nodding and ' \
               'maintaining eye contact to physically show that you are listening!) '+ 'A stands for AFFIRM ' \
               'and acknowledge (Let them know that their feelings are valid!) ' + 'R stands for RESPOND ' \
               '(Once you acknowledge their POV, make sure to respond respectfully ' + 'and finally, ' \
               'A stands for ADD (This is when either of you can add to the conversation ' + 'to work towards ' \
               'a solution together! ' + 'Try it out, and let me know how it goes!'

    def finish_looking_for_solution (self):
        return 'Here are some things that have worked in the past:' + ' Try having the conversation one ' \
                                                                      'more time and look to find a compromise.' +' ' \
    ...
    def finish_looking_for_room_change(self):
        return 'I am sorry that your roommate situation has got this bad. Here is the ResED contact info' +\
        ' which you can use to request a room change: resed@oxy.edu'

    def finish_looking_for_personal_issue(self):
        return 'I understand that it can be really scary and difficult to have these conversations,'+\
    ...
    def finish_looking_for_roommate_issue(self):
        return 'I am sorry things have become so difficult with your roommate. They seem' +\
